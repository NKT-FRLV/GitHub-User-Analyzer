...
const AIFeedbackChat = ({
	...
	const isFileSelectionModalOpen = useRepoStore(
		(state) => state.isFileSelectionModalOpen
	);
	const setFileSelectionModalOpen = useRepoStore(
		(state) => state.setFileSelectionModalOpen
	);
	const setSelectedFileInStore = useRepoStore(
	...
	const handleAskAi = useCallback(
		async (callback: React.Dispatch<React.SetStateAction<string>>) => {
			if (!repoName) {
			...
			if (selectedFileFromStore) {
				performAiAnalysis(
					selectedFileFromStore.path,
					selectedFileFromStore.content,
					selectedFileFromStore.type,
					callback
				);
			} else {
		...
		[repoName, selectedFileFromStore, setFileSelectionModalOpen, performAiAnalysis]
	);
	...
	const handleOpenCostInfoDialog = useCallback(
		() => setIsCostInfoDialogOpen(true),
		[]
	);
	const handleCloseCostInfoDialog = useCallback(
		...
		>
			...
			>
				...
				{selectedFileFromStore && (
					...
					>
						<Typography variant="body2" color="primary">
							Selected file:{" "}
							<strong>{selectedFileFromStore.path}</strong>
						</Typography>
						<Button
							size="small"
				...
				<FilePreviewModal
					...
					setClose={handleCloseFileDialog}
					fileType={fileType}
					fileContent={fileContent}
				/>
				...
				{/* Cost Info MODAL */}
				<CostInfoModal
					isOpen={isCostInfoDialogOpen}
					setClose={handleCloseCostInfoDialog}
					costInfo={costInfo}
					isSmallScreen={isSmallScreen}
				/>
